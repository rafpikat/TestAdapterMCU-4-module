...
public:
  ...
  static const uint8_t REG_DATA =
      0x3; // Data register (16-bit, conversion result)
  ...
  static const uint8_t MODE_ZERO_SCALE_CAL =
      0x2; // Zero-scale system calibration
  static const uint8_t MODE_FULL_SCALE_CAL =
      0x3; // Full-scale system calibration
  ...
  static const uint8_t UNIPOLAR = 0x1;
  static const uint8_t BIPOLAR = 0x0;
  ...
  typedef enum {
    ...
    CS_4 = 3  ///< Chip Select 4
  } CS_t;
  AD770X(double vref, spi_host_device_t spi_host, gpio_num_t cs_pin_1,
         ...
         gpio_num_t reset_pin_4);
  ~AD770X();
  void setNextOperation(uint8_t reg, uint8_t channel, uint8_t readWrite,
                        CS_t cs);
  void writeClockRegister(uint8_t CLKDIS, uint8_t CLKDIV,
                          uint8_t outputUpdateRate, CS_t cs);
  void writeSetupRegister(uint8_t operationMode, uint8_t gain, uint8_t unipolar,
                          uint8_t buffered, uint8_t fsync, CS_t cs);
  ...
  void init(uint8_t channel, uint8_t clkDivider, uint8_t polarity, uint8_t gain,
            uint8_t updRate, CS_t cs);
  ...
  // Hàm debug kiểm tra phần cứng SPI
  void debugHardware(CS_t cs);

...
uint8_t UartDriver::calculateChecksum(const uint8_t* message, size_t data_start, size_t data_end) {
    uint8_t check_xor = 0;
...
bool UartDriver::send(float value, uint8_t cmd_id, uint8_t id_board) {
    ...
    if (sent_bytes != static_cast<int>(index)) {
        ESP_LOGE(TAG, "Failed to send all bytes, sent: %d, expected: %d", sent_bytes, index);
        return false;
    }
    return true;
}
...
void UartDriver::init() {
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_PORT_NUM, BUF_SIZE * 2, BUF_SIZE * 2, 20, &uart_queue_, 0));
    ...
    while (true) {
        if (xQueueReceive(driver->uart_queue_, &event, portMAX_DELAY)) {
            if (event.type == UART_DATA) {
                while (true) {
                    uint8_t byte;
                    int len = uart_read_bytes(UART_PORT_NUM, &byte, 1, 20 / portTICK_PERIOD_MS);
                    if (len <= 0) break;
                    ...
                    if (index == 4) {
                        ...
                        if (data_length == 0 || data_length > 12) { // Giới hạn để tránh tràn buffer
                    ...
                    // Kiểm tra khi nhận đủ bản tin (2 start bytes + 1 CmdID + 1 Length + data_length + 1 Checksum)
                    if (index == 4 + buffer[3] + 1) {
                        // Kiểm tra Checksum
                        uint8_t received_checksum = buffer[index - 1];
                        uint8_t calculated_checksum = driver->calculateChecksum(buffer.data(), 4, 4 + buffer[3]);
                        if (received_checksum != calculated_checksum) {
                            ESP_LOGE(TAG, "Checksum mismatch: received 0x%02X, calculated 0x%02X", received_checksum, calculated_checksum);
                            index = 0;
                            start_found = false;
                            continue;
                        }
